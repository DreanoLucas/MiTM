...
def create_json_log(liste:list, filename:str)->None:
    ...
    try:
        ...
        jsonfile = json.loads(fichier.read())
        fichier.close()
    except:
        pass
    ...
    fichier.write(json.dumps(jsonfile, indent=4))
    fichier.close()
...
def create_sql_log(liste:list, filename:str):
    ...
    for i in  liste:
        values = f"'{i[0]}','{i[1]}','{i[2]}','{i[3]}'"
        cursor.execute(f"INSERT INTO sql_log (date, ip, methode, URI) VALUES ({values})")
